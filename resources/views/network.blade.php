@extends('layouts.default')
@section('title', 'Network')
@section('hero')
    <div class="row hero bg-image">
        <div class="col s12">
            <h4 class="header">The Network</h4>
        </div>
    </div>
@endsection
@section('content')
<div class="container">
    <div class="row">
        <div class="col s12">
            <p>The key to success in this industry is the network. At Sports Business Solutions, we feel as if that's one of our strengths. With more than 10 years of industry experience we have established relationships built with individuals and hiring managers employed with the teams below.</p>
            <p>As a potential recruiting solution, we feel confident we have the right experience and contacts to find your next sports industry leader. If you're interested in learning about our recruiting solutions click here</p>
            <p>If you're a job seeker interested in getting an introduction to a hiring manager with these teams, click here, fill out a career profile and tell us more about yourself and where you'd like to work in sports.</p>
        </div>
    </div>
    <div class="row">
        <div class="col s12 m3">
            <ul class="large-text">
                <li><b>NCAA (28)</b></li>
                <li>Arizona</li>
                <li>Arizona State</li>
                <li>Aspire Group</li>
                <li>Cal Berkeley</li>
                <li>Colorado State</li>
                <li>Eastern Michigan</li>
                <li>Georgetown</li>
                <li>IMG College</li>
                <li>Learfield</li>
                <li>Legends</li>
                <li>Louisville</li>
                <li>Memphis</li>
                <li>Miami (OH)</li>
                <li>Minnesota</li>
                <li>NC State</li>
                <li>New Mexico</li>
                <li>Notre Dame</li>
                <li>Oklahoma</li>
                <li>Pac 12 Conference</li>
                <li>Rose Bowl</li>
                <li>Santa Clara</li>
                <li>Seaton Hall</li>
                <li>Southern Illinois</li>
                <li>Texas</li>
                <li>UC Davis</li>
                <li>UCLA</li>
                <li>USC</li>
                <li>UTSA</li>
            </ul>
        </div>
        <div class="col s12 m3">
            <ul class="large-text">
                <li><b>NBA (27)</b></li>
                <li>Atlanta Hawks</li>
                <li>Boston Celtics</li>
                <li>Charlotte Hornets</li>
                <li>Cleveland Cavaliers</li>
                <li>Detroit Pistons</li>
                <li>Denver Nuggets</li>
                <li>Golden State Warriors</li>
                <li>Houston Rockets</li>
                <li>Indiana Pacers</li>
                <li>Los Angeles Clippers</li>
                <li>Los Angeles Lakers (AEG)</li>
                <li>Miami Heat</li>
                <li>Milwaukee Bucks</li>
                <li>Minnesota Timberwolves</li>
                <li>NBA League Office</li>
                <li>New Orleans Pelicans</li>
                <li>New York Knicks</li>
                <li>Oklahoma City Thunder</li>
                <li>Orlando Magic</li>
                <li>Philadelphia 76ers</li>
                <li>Phoenix Suns</li>
                <li>Portland Trailblazers</li>
                <li>Sacramento Kings</li>
                <li>San Antonio Spurs</li>
                <li>Staples Center (AEG)</li>
                <li>Toronto Raptors</li>
                <li>Utah Jazz</li>
                <br/>
                <li><b>WNBA (5)</b></li>
                <li>Indiana Fever</li>
                <li>Los Angeles Sparks</li>
                <li>Phoenix Mercury</li>
                <li>Washington Mystics</li>
                <li>WNBA League Office</li>
                <br/>
                <li><b>NBADL (3)</b></li>
                <li>D-League Office</li>
                <li>Iowa Energy</li>
                <li>Santa Cruz Warriors</li>
            </ul>
        </div>
        <div class="col s12 m3">
            <ul class="large-text">
                <li><b>MLB (19)</b></li>
                <li>Arizona Diamondbacks</li>
                <li>Boston Red Sox</li>
                <li>Chicago Cubs</li>
                <li>Cincinnati Reds</li>
                <li>Colorado Rockies</li>
                <li>Houston Astros</li>
                <li>Los Angeles Angels</li>
                <li>Los Angeles Dodgers</li>
                <li>Miami Marlins</li>
                <li>Minnesota Twins</li>
                <li>MLB League Office</li>
                <li>New York Yankees</li>
                <li>New York Mets</li>
                <li>Oakland A’s</li>
                <li>Pittsburgh Pirates</li>
                <li>San Diego Padres</li>
                <li>San Francisco Giants</li>
                <li>Texas Rangers</li>
                <li>Washington Nationals</li>
            </ul>
        </div>
        <div class="col s12 m3">
            <ul class="large-text">
                <li><b>NFL (18)</b></li>
                <li>Atlanta Falcons</li>
                <li>Carolina Panthers</li>
                <li>Cincinnati Bengals</li>
                <li>Cleveland Browns</li>
                <li>Dallas Cowboys</li>
                <li>Detroit Lions</li>
                <li>Houston Super Bowl</li>
                <li>Houston Texans</li>
                <li>Jacksonville Jaguars</li>
                <li>Kansas City Chiefs</li>
                <li>Los Angeles Rams</li>
                <li>Legends</li>
                <li>Miami Dolphins</li>
                <li>Oakland Raiders</li>
                <li>San Diego Chargers</li>
                <li>San Francisco 49ers</li>
                <li>Tampa Bay Buccaneers</li>
                <li>Tennessee Titans</li>
                <br/>
                <li><b>AFL (3)</b></li>
                <li>Arizona Rattlers</li>
                <li>Canton Charge</li>
                <li>Los Angeles Kiss</li>
            </ul>
        </div>
    </div>
    <div class="row">
        <div class="col s12 m3">
            <ul class="large-text">
                <li><b>NHL (15)</b></li>
                <li>Arizona Coyotes</li>
                <li>Carolina Hurricanes</li>
                <li>Colorado Avalanche</li>
                <li>Columbus Blue Jackets</li>
                <li>Dallas Stars</li>
                <li>Detroit Red Wings</li>
                <li>Florida Panthers</li>
                <li>Minnesota Wild</li>
                <li>New Jersey Devils</li>
                <li>New York Rangers</li>
                <li>New York Islanders</li>
                <li>Philadelphia Flyers</li>
                <li>San Jose Sharks</li>
                <li>Tampa Bay Lightning</li>
                <li>Washington Capitals</li>
                <br/>
                <li><b>AHL (4)</b></li>
                <li>AHL League Office</li>
                <li>Lake Erie Monsters</li>
                <li>San Diego Gulls</li>
                <li>San Jose Barracuda</li>
                <br/>
                <li><b>ECHL (1)</b></li>
                <li>Tulsa Oilers</li>
            </ul>
        </div>
        <div class="col s12 m3">
            <ul class="large-text">
                <li><b>MLS (15)</b></li>
                <li>Colorado Rapids</li>
                <li>Columbus Crew</li>
                <li>DC United</li>
                <li>FC Dallas</li>
                <li>Houston Dynamo</li>
                <li>Los Angeles Football Club</li>
                <li>Los Angeles Galaxy</li>
                <li>MLS League Office</li>
                <li>NYCFC</li>
                <li>NY Red Bulls</li>
                <li>Orlando City FC</li>
                <li>Portland Timbers</li>
                <li>San Jose Earthquakes</li>
                <li>Seattle Sounders</li>
                <li>Sporting Kansas City</li>
                <br/>
                <li><b>USL (4)</b></li>
                <li>Arizona United</li>
                <li>Colorado Springs Switchbacks</li>
                <li>Jacksonville Armada</li>
                <li>Sacramento Republic FC</li>
                <br/>
                <li><b>NASL (1)</b></li>
                <li>Minnesota United FC</li>
            </ul>
        </div>
        <div class="col s12 m3">
            <ul class="large-text">
                <li><b>MiLB (4)</b></li>
                <li>Akron Rubber Ducks</li>
                <li>Salem Red Sox</li>
                <li>Sacramento River Cats</li>
                <li>Indianapolis Indians</li>
            </ul>
        </div>
        <div class="col s12 m3">
            <ul class="large-text">
                <li><b>Motorsports (2)</b></li>
                <li>Circuit of the Americas F1 Racetrack</li>
                <li>International Speedway Corp.</li>
            </ul>
        </div>
    </div>
</div>
@endsection
