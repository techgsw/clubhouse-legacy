    ...
    <form id="unsubscribe" method="post" action="/unsubscribe/{{ $profile->email_unsubscribe_token }}" enctype="multipart/form-data">
        ...
        <div class="row">
            @if (!$email_preference_new_content && !$profile->email_preference_marketing && !$profile->email_preference_new_job)
                <div class="col s12">
                    <p><h5><strong>You are already unsubscribed from all content notifications.</strong></h5> If you are still receiving emails, please forward them to <a href="mailto:clubhouse@sportsbusiness.solutions">clubhouse@sportsbusiness.solutions</a> and we will fix the issue.</p>
                </div>
            @else
