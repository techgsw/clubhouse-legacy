@component('emails.layout')
    @slot ('title')
        Webinar Purchase - theClubhouse
    @endslot
    @slot('body')
        <p>Hi {{ ucwords($user->first_name) }},</p>
        <p>Thanks for signing up for our upcoming webinar {{ $prduct_option->product->name }} on {{ $product_option->name }} {{ $product_option->description }}.</p>
        <p>Your card has been charged {{ money_format('%2n', $product_option->price) }}</p>
