            ...
            @can ('edit-inquiry', $inquiry)
                ...
                <div style="margin: 4px 0;">
                    ...
                    @if ($inquiry->pipeline_id == 1 && !$contact)
                        <button data-action="" data-type="{{ $contact ? 'contact' : 'user' }}" data-pipeline-id="{{$inquiry->pipeline_id}}" data-id="{{ $inquiry->id }}" data-move="halt" class="flat-button small blue {{$inquiry->status == 'halted' ? 'inverse' : ''}} negative-pipeline-modal-button default-comm"><i class="fa fa-envelope"></i>&nbsp;<i class="fa fa-thumbs-down"></i></button>
                    @endif
                    ...
                    @if ($inquiry->pipeline_id < 2 && $contact)
                        ...
                        <button data-action="inquiry-pipeline" data-comm-type="none" data-type="{{ $contact ? 'contact' : 'user' }}" data-pipeline-id="{{$inquiry->pipeline_id}}" data-id="{{ $inquiry->id }}" data-move="forward"  class="flat-button small blue no-comm"><i class="fa fa-thumbs-up"></i> {{$contact && $inquiry->job->recruiting_type_code !== 'passive' ? 'None' : '' }}</button>
                    @elseif ($contact) 
                        <button data-action="inquiry-pipeline" data-comm-type="none" data-type="{{ $contact ? 'contact' : 'user' }}" data-pipeline-id="{{$inquiry->pipeline_id}}" data-id="{{ $inquiry->id }}" data-move="forward" class="flat-button small {{$inquiry->pipeline_id > 1 && $inquiry->recruiting_code_type == 'passive' ? 'hidden' : ''}} no-comm {{$inquiry->pipeline_id == 6 ? 'gray' : 'blue'}}" {{$inquiry->pipeline_id == 6 ? 'disabled' : ''}}><i class="fa fa-thumbs-up"></i> {{$contact && $inquiry->job->recruiting_type_code !== 'passive' ? 'None' : '' }}</button>
                    @endif
