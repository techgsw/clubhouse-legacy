    ...
    <div class="col s9 m10 info">
        @can ('edit-inquiry', $inquiry)
            <div class="hide-on-small-only float-right controls">
                ...
                <button data-action="inquiry-pipeline" data-comm-type="cold"{{ $inquiry->pipeline_id < 6 ? '' : 'disabled' }} data-type="{{ $inquiry instanceof App\ContactJob ? 'contact' : 'user' }}" data-pipeline-id="{{$inquiry->pipeline_id}}" data-id="{{ $inquiry->id }}" data-move="forward" class="flat-button small {{$inquiry instanceof App\ContactJob && $inquiry->job->recruiting_type_code == 'passive' ? 'hidden' : ''}} {{ $inquiry->pipeline_id < 6 ? 'blue' : 'gray' }} cold-comm"><i class="fa fa-envelope {{$inquiry->pipeline_id > 1 || !$inquiry instanceof App\Inquiry ? 'hidden' : ''}}"></i>{{$inquiry->pipeline_id < 2 || !$inquiry instanceof App\Inquiry && $inquiry->job->recruiting_code_type == 'passive'  ? '&nbsp;' : ''}}<i class="fa fa-thumbs-up"></i><span class="thumbs-up-text">{{ $inquiry->pipeline_id < 2 && !$inquiry instanceof App\Inquiry ? ' Cold' : '' }}</span></button>
