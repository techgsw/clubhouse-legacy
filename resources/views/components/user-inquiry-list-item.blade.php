    ...
    <div class="col s9 m10 info">
        @can ('edit-inquiry', $inquiry)
            <div class="hide-on-small-only float-right controls">
                ...
                @if ($inquiry->reason)
                    <button type="button" id="pipeline-label-{{ $inquiry->id }}" style="display:{{!$inquiry ? '' : ($inquiry->pipeline_id != 1  ? '' : 'none')}};" class="flat-button red small input-control" input-id="step">{{$inquiry->reason}}</button>
                @endif   
