...
class UploadContacts extends Command
{
    ...
    public function handle()
    {
        ...
        while (($row = fgetcsv($handle, ",")) !== false) {
            ...
            $contact = Contact::with('address')->where('first_name', '=', $first_name)
                                               ->Where('last_name', '=', $last_name)
                                               ->get();
                                            // ->toSql();
            // You could use this to get the Contact first_name if it had existed
            // $first_nameExists = Contact::where('first_name', '=', $first_name)->get();
            // $last_nameExists = Contact::where('last_name', '=', $last_name)->get();

            //update the contact address correcrtly
            //google join for DB
            //"Laravel eloquent relationships
            if (count($contact) > 0) {
                echo $first_name . ' ' . $last_name . " already exists.\n";
                // TODO Update?
            ...
            if(count($contact) > 1){
                echo "Potential duplicate found - " . $first_name . " " . $last_name;
            }
            // dd($contact);
            $sbs_reps = array();
